    ...
    for dataset_set_index in range(5):
        ...
        for epoch in range(params.max_iter):
            ...
            for idx in range(train_N):
                ...
                pred_res, features,_,_,_ = net(skill, answer)
                ...
                pred_res, features,_,_,_ = net(skill, answer, p_adv)
        ...
        #loss_vision
        with open("2015filter_Val_Loss.json", "w+") as f:
        ...
        with torch.no_grad():
            for idx in range(test_N):
                ...
                pred_res, features,skill_embedding,skill_embedding_filter,res = net(skill, answer)

                # if idx == test_N-1:
                #     a.append(skill_embedding)
                #     b.append(skill_embedding_filter)
                loss, y_pred, y_true = kt_loss(pred_res, answer)
                ...
                if idx==0:
                    # 创建一个字典，将张量与名称关联
                    data_to_save = {
                        'tensor1': torch.tensor(test_batch_skill[0, :49]).long(),
                        'tensor2': torch.tensor(test_batch_answer[0, :49]).long(),
                        'tensor3': res[0,:,:].detach().cpu()
                    }
            ...
            gc.collect()
            torch.cuda.empty_cache()
        # with open("hotmap.json", "w+") as f:
        #     json.dump(res[0,:,:5].T,f)  
